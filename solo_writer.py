...
import datetime, glob, os, smtplib, subprocess, sys, traceback, json
...
def apply_theme(theme_name):
    ...
    root.tk_setPalette(background=theme["window_bg"],
...
def init_browser_frame():
    ...
    # Bind keys for file loading and deletion.
    ...
    populate_file_list()  # Populate the listbox with files.
...
def toggle_theme(event=None):
    ...
    instr_lbl = tk.Label(info_frame, text="Select Theme (Arrow keys, Enter to apply, Escape to cancel):",
                         font=("Helvetica", 14), bg=current_colors["window_bg"], fg=current_colors["text_fg"])
    instr_lbl.pack(padx=10, pady=5)

    theme_var_local = tk.StringVar(value=current_theme)
    rb_widgets = []
    for t in themes:
        rb = tk.Radiobutton(info_frame, text=t, variable=theme_var_local, value=t,
                            font=("Helvetica", 14), bg=current_colors["window_bg"], fg=current_colors["text_fg"],
                            selectcolor=current_colors["window_bg"], indicatoron=True)
